...
[![Build Status](https://travis-ci.org/mediumTaj/dot-net-sdk.svg?branch=develop)](https://travis-ci.org/mediumTaj/dot-net-sdk) [![Coverage Status](https://coveralls.io/repos/github/mediumTaj/dot-net-sdk/badge.svg?branch=develop)](https://coveralls.io/github/mediumTaj/dot-net-sdk?branch=develop)
...
1. Create a `Config.json` file in your `appdata` directory (`bin/Debug/appdata` by default).
2. Credentials should be formatted using the following template.
```
{
    "m_TimeOut": 120.0,
  ...
  "m_Credentials": [
    {
      "m_ServiceID": "[service-id]",
      "m_URL": "[service-url]",
      "m_User": "[service-username]",
      "m_Password": "[service-password]",
      "m_Apikey": "[service-apikey]"
    },
    {
      "m_ServiceID": "[service-id]",
      "m_URL": "[service-url]",
...
```
Multiple services can be configured in the m_Credentials object and user definied variables can be added to m_Variables object. Service IDs can be found in each particular service's abstraction under `SERVICE_ID`. Credential loading can be disabled or revised in Config.cs.

2. If you have configured the services already, complete the following steps. Otherwise, go to step 3.
    1. Log in to Bluemix at https://bluemix.net.
    2. Click a service.
    3. Click **Service Credentials** and click the **View Credentials** action for the service credential to be used.
    4. Add the service credential URL and password, username and/or apikey if applicable to the Config.json file.
    5. Repeat steps 1 - 4 for each service you want to use.

3. If you need to configure the services that you want to use, complete the following steps.
    1. Log in to Bluemix at https://bluemix.net.
    2. Navigate to **Catalog** on your Bluemix account and select the **Watson** category.
    3. Click a service to navigate to the service page.
    4. Selecta a service tier and name the service and credentials if desired.
    5. Click **Create** to create a service instance.
    6. Click **Service Credentials** and click the **View Credentials** action for the service credential to be used.
    7. Add the service credential URL and password, username and/or apikey if applicable to the Config.json file.
    8. Repeat steps 2 - 7 for each service you want to use.
...
[latest_release]: https://github.com/watson-developer-cloud/dotnet-sdk/releases/latest
[bluemix_registration]: http://bluemix.net/registration
[get_visual_studio]: https://www.visualstudio.com/vs/community/

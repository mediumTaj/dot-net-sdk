...
namespace TestDebug
{
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      //{
      //  Log.Debug("Debug Program", "received! {0}", entityData);
      ...
      //string createdClassifierName = "dot-net-integration-test-classifier";
      //string createdClassifierLanguage = "en";
      //string trainingDataPath = Constants.Path.APP_DATA + "/naturalLanguageClassifierTrainingData.json";
      //string classifyText = "Will it rain today?";
      //ClassifierData data = new ClassifierData();
      //data.Load(trainingDataPath);

      //createdClassifierName = createdClassifierName + DateTime.Now.ToString();
      ...
      //}))
      //{
      //  Log.Debug("Progam", "Failed to train classifier!");
      //}
      ...
      //alchemyDataNews.GetNews((NewsResponse newsData, string data) => { newsResponse = newsData; Log.Debug("TestAlchemyDataNews", "newsResponse: {0}", newsResponse); }, returnFields, queryFields);

      //Log.Debug("TestAlchemyDataNews", "newsResponse: {0}", newsResponse);
      //Assert.AreNotEqual(newsResponse, null);

      AutoResetEvent autoEvent = new AutoResetEvent(false);
      string classifierID = string.Empty;
      naturalLanguageClassifier.FindClassifier("Test Natural Language Classifier/12/7/2016 10:41:19 AM", (Classifier classifier) =>
      {
      ...
      autoEvent.WaitOne();
      Log.Debug("test", "classifierID: {0}", classifierID);



      ClassifyResult classifyResult = null;
      if (!naturalLanguageClassifier.Classify(classifierID, "Is it raining?", (ClassifyResult result) =>
      {
      ...
      autoEvent.WaitOne();
      Log.Debug("test", "result: {0}", classifyResult.classifier_id);
      Console.ReadLine();
